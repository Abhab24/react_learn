...
//.searchText: Holds the text entered in the search input.

  const [listofRestaurants, setlistofRestaurants] = useState([]);//modifying this 1 time only wen im getting data from API ,this will be used to filter out restaurants
  ...
  useEffect(() => {//anything inside this fn will be called after rendering the function
    try {
  ...
  const fetchData = async () => {//fetch data fn
    const data = await fetch(//fetch fn returns a promise and we resolve this fn using async await
...
//populating ui with json obtained
    json.data.cards.forEach((singleCard) => {
      if (singleCard.card.card.gridElements) {
        //to check validity of data as live data keeps on changing so to avoid errors
        console.log(singleCard.card.card);
        //updating both the state variables with data obtained from API
        setlistofRestaurants(singleCard?.card?.card?.gridElements?.infoWithStyle?.restaurants);
        setfilteredRestaurants(singleCard?.card?.card?.gridElements?.infoWithStyle?.restaurants)
      }
    });
  };
//conditional rendering
  if (listofRestaurants.length === 0) {//for better user experience while waiting for data to get loaded placeholders are visible on ui
  ...
  return (         //this filter div has search bar,top rated filter
    <div className="body">
      <div className="filter">
        <div className="search">
          <input //search input ,input is controlled by searchtext state variable
            ...
            onChange={(e) => {//(this loop is for changing value of input on ui for display)When the input value changes (onChange), the setsearchText function is called to update the searchtext variable and state of component
              setsearchText(e.target.value);
            }}
          />
          <button
            onClick={() => {//if search button gets clicked then filtering fn is called
              console.log("Search Button Clicked");
              console.log(searchText);

              const filteredRestaurants = listofRestaurants.filter( //filters all the restaurant names which include searched and then updating listofrestaurants with that res data
      ...
      <div className="resContainer">
        {filteredRestaurants.map((restaurant, index) => (//now well also render using filtered restaurants
        ...
        //but doing restaurant.info gives us access to data of each card (1 step andr) so we use it as a prop for rescard component
          <ResCard key={index} resData={restaurant.info} />//resData is prop we are passing to rescard component
        ))}
      </div>
    </div>
  );
